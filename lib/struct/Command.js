...
class Command extends Base {
  ...
  onCooldown() {
    if (
      this.cooldownManager.has(
        this.parent ? `${this.parent}-${this.name}` : this.name
      )
    ) {
      return this.cooldownManager.get(
        this.parent ? `${this.parent}-${this.name}` : this.name
      );
    } else {
      return false;
  ...
  toJSON(slash = false) {
    ...
    if (data.parent) {
      let da = this.client.defConfig.slashCommand.parentCommands.find(
        (c) => c.dir === data.parent
      );

      if (!da || !da.name || !da.description) {
        this.log.typer(
          "Name and description must be defined for category " + data.parent
        );
      }
    ...
    if (data.memberPermissions?.length > 0) {
      data.default_member_permissions = data.memberPermissions
        .map((e) => {
  ...
  replaceOptions(opts) {
    for (const option of opts) {
      ...
      if (option.memberPermissions?.length > 0) {
        option.default_member_permissions = option.memberPermissions
          .map((e) => {
            if (!PermissionFlagsBits[e]) {
              this.log.typer(`Invalid Permission ${e} in ${data.name} command`);
            }
