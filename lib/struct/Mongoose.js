...
class Mongoose extends Base {
  constructor(options = {}) {
    ...
    this.db.connection.on("error", (e) => {
      this.log.error(e.message, "MONGOERROR");
    });
  ...
  init() {
    ...
    this.log.wait("Loading Mongo...");
    this.db.connect(this.uri, this.settings).catch((error) => {
      this.log.warn(error.message, "MONGOERROR");
    });
  ...
  add(model) {
    ...
    this[model] = this.db.models[model] || this.model(model, module);
  }
