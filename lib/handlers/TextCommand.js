...
class TextHandler extends Base {
  ...
  async handle(msg, prefix = this.config.bot.prefix || "$") {
    ...
    if (
      msg.content.match(new RegExp(`^<@!?${msg.client.user.id}>( |)$`))
    ) {
      return msg.reply(`My prefix is ${prefix}.`)
    }

    if (msg.content.startsWith(msg.client.user.username)) {
      prefix = msg.client.user.username;
