...
class SlashHandler extends Base {
  ...
  async handle(int) {
    if (int.commandType !== this.client.consts.ApplicationCommand.ChatInput)
      return;
    ...
    const { access, reasons } = await check(command, int);
    ...
    if (cooldown) {
      return int.follow({
        content: `This command is currently on cooldown for ${ms(
          cooldown.time + cooldown.cooldown - Date.now()
        )}`,
      });
    }

    try {
      await command.run(this.client, int, options);
      ...
      int.client.log.error("Error While Executing " + command.name);
      console.log(e);
