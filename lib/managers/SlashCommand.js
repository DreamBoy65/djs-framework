...
class SlashCommandManager extends Base {
  constructor(client, config) {
    ...
    this.client = client;
    this.config = config;
    this.commands = client.collections
      .add("slashCommands")
      .get("slashCommands");
  ...
  registerCommands() {
    ...
    for (const command of this.commands) {
      ...
      let mmd = cmds.find((c) => c.name === cmd.name);
      if (mmd) {
        mmd.options.push(cmd.options[0]);
      } else {
        cmds.push(cmd);
      }
    }
    ...
    if (this.loadGlobal) {
      this.log.warn(
        "The slash commands might take upto 1hour to get loaded globally"
      );
  ...
  async loadAll() {
    ...
    if (!this.functions.isDir(this.dir))
      return this.log.typer(`${this.dir} is not a directory`);

    let files = await this.functions.readdirSync(
      `${process.cwd()}/${this.dir}`
    );
  ...
  removeCommand(name) {
    if (!this.commands.has(name))
      return this.log.typer(`No command found with name: ${name}`);
